...
const AddReview = ({ application, scholarshipDetails, onClose }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await axios.post(
        "https://s-server-two.vercel.app/reviews",
        {
...
const MyApplications = () => {
  ...
  // ✅ Fetch Applications (GET is public, no token needed)
  const fetchApplications = () => {
    if (user?.email) {
      axios
        .get(
          `https://s-server-two.vercel.app/applications?email=${encodeURIComponent(
            user.email
          )}`
        )
  ...
  // ✅ Cancel Application (requires token)
  const handleCancel = async (id) => {
    ...
    if (confirm.isConfirmed) {
      try {
        const token = await user.getIdToken();
        await axios.delete(`https://s-server-two.vercel.app/applications/${id}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  const handleAddReviewClick = async (app) => {
    try {
      const res = await axios.get(
        `https://s-server-two.vercel.app/scholarships/${app.scholarshipId}`
      );
