...
const MyApplications = () => {
  ...
  const handleEdit = (status, id) => {
    if (status !== "pending") {
      ...
      navigate(`/dashboard/UpdateApplicationForm/${id}`); // Make sure to create this route & page
    }
  };

  return (
    <div className="px-4 py-6">
      <h2 className="text-xl md:text-2xl font-bold mb-4 text-center">My Applications</h2>
                ...
                <tr key={app._id} className="border-t text-sm md:text-base">
                  ...
                  <td className="p-2">
                    <div className="flex flex-col md:flex-row gap-2">
                      <button
                        className="bg-green-600 text-white px-3 py-1 rounded text-xs"
                        onClick={() => navigate(`/dashboard/scholarship/${app.scholarshipId}`)}
                      >
                        Details
                      </button>
